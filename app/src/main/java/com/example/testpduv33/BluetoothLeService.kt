...
class BluetoothLeService():Service() {
    ...
    companion object{
        ...
        const val ACTION_DATA_WRITE="com.example.bluetooth.le.ACTION_DATA_WRITE"
    ...
    private fun broadcastUpdate(action: String, characteristic: BluetoothGattCharacteristic){
        ...
        if (action== ACTION_DATA_WRITE){//write only
...
//16,0,60,0,0,0,-112,1
                ...
                GattAttributes.machine_status->{
                    ...
                    Log.d(TAG,"the machine status is $data")
                }
                GattAttributes.meter_parameter->{
                    val data=characteristic.value.toHexString()
                    intent.putExtra(EXTRA_DATA,data)
                    intent.putExtra(CHARACTERISTIC,GattAttributes.mMeterVersion)
                    Log.d(TAG,"the parameter is $data")
                }
                GattAttributes.read_all_charging_record->{
                    val byteData=characteristic.value as ByteArray
                    intent.putExtra(CHARACTERISTIC,GattAttributes.mAllChargingReport)
                    intent.putExtra(EXTRA_DATA,byteData)
                }
                GattAttributes.activate_power->{
                    val data=(characteristic.value[0].toInt() and (0xFF))

                    intent.putExtra(CHARACTERISTIC,GattAttributes.mPowerRead)
                    if (data==1){
                        intent.putExtra(EXTRA_DATA, getString(R.string.its_on))
                        Log.d(TAG, "$data, activate power")
                    }else if (data==0){
                        intent.putExtra(EXTRA_DATA, getString(R.string.its_off))
                        Log.d(TAG, "$data, activate power")
